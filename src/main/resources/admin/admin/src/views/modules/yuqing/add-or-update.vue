<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="帖子标题" prop="wenzhangbiaoti">
          <el-input v-model="ruleForm.wenzhangbiaoti" 
              placeholder="帖子标题" clearable  :readonly="ro.wenzhangbiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="帖子标题" prop="wenzhangbiaoti">
              <el-input v-model="ruleForm.wenzhangbiaoti" 
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                            ...
                                                            this.$http({
              url: `option/wenzhangfenlei/fenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
	...
	this.ruleForm.wenzhangneirong = this.ruleForm.wenzhangneirong.replace(reg,'../../../springboot60zv5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
